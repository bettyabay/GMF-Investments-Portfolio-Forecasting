...
class DataPreprocessor:
    ...
    def get_data(self, start_date, end_date, symbols):
        ...
        """
        # Initialize empty dictionary to store file paths for each symbol
        # For TSLA example: data_paths = {} initially
        data_paths = {}
        
        # Loop through each symbol (e.g., 'TSLA') in the provided symbols list
        for symbol in symbols:
            try:
                # Print status message, e.g.: "Fetching data for TSLA from 2023-01-01 to 2023-12-31..."
                print(f"Fetching data for {symbol} from {start_date} to {end_date}...")
                
                # Fetch historical data using pynance
                # For TSLA, gets daily OHLCV data for specified date range
                data = pn.data.get(symbol, start=start_date, end=end_date)
                
                # Create file path, e.g.: "/data/TSLA.csv"
                file_path = os.path.join(self.data_dir, f"{symbol}.csv")
                
                # Save DataFrame to CSV file
                # Creates TSLA.csv with columns: Date,Open,High,Low,Close,Volume,Adj Close
                data.to_csv(file_path)
                
                # Store file path in dictionary
                # data_paths['TSLA'] = '/data/TSLA.csv'
                ...
                # e.g., "Data format issue for TSLA: Invalid date format"
                ...
                # e.g., "Failed to fetch data for TSLA: Connection timeout"
